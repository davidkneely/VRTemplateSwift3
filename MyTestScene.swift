...
class MyTestScene : NSObject, VRControllerProtocol, SCNPhysicsContactDelegate {
    ...
    func generateSphereFocalPointAt(_ location: SCNVector3, color: UIColor, size: CGFloat){
        ...
        sphere.physicsBody = SCNPhysicsBody.static()
        boxes.addChildNode(sphere)
        
        let remove = SCNAction.removeFromParentNode()
        let delay = SCNAction.wait(duration: 20)
    ...
    func prepareFrame(with headTransform: GVRHeadTransform) {
        ...
        if usingExtendedGamePad == true {
            ...
            let actionH = SCNAction.rotate(by: CGFloat(GLKMathDegreesToRadians(rotateBy)), around: SCNVector3Make(0, 1, 0), duration: 0.1)
...
//        }
       ...
       let position = cameraNode.position
        ...
        if ((control?.buttonBPressed) == true) {
            ...
            let line = CylinderLine(parent: boxes, v1: positionLine, v2: projected, radius: 0.2, radSegmentCount: 10, color: UIColor.red)
            ...
            bullet.materials.first?.diffuse.contents = UIColor.yellow
            ...
            let shootAction = SCNAction.move(to: projected, duration: 2)
        ...
        if control?.rightTriggerPressed == true {
            ...
            shootBullet(pos, to: projected, color: UIColor.red, size: 0.25)
        }
        
        if control?.leftTriggerPressed == true {
            let pos = positionLine
            shootBullet(pos, to: projected, color: UIColor.blue, size: 0.25)
        }
        
        let hits = boxes.hitTestWithSegment(from: position, to: p2, options: [SCNHitTestOption.firstFoundOnly.rawValue: true]);
        ...
        focusedNode?.geometry?.materials.first?.emission.contents = UIColor.yellow.withAlphaComponent(0.3)

    }
    
    func shootBullet(_ from: SCNVector3, to:SCNVector3, color: UIColor, size : CGFloat) {
    ...
    func eventTriggered() {
        ...
        if let pos = focusedNode?.position {
            doingSomething = true
            let action = SCNAction.move(to: pos, duration: 0.5)
            action.timingMode = .easeOut
            let delay = SCNAction.wait(duration: 0)
...
//        infoLabelNode.geometry = SCNBox(width: 1, height: 1, length: 1, chamferRadius: 0.2)
        ...
        backSphereN.physicsBody = SCNPhysicsBody.static()
        ...
        backSphereS.physicsBody = SCNPhysicsBody.static()
        ...
        backSphereE.physicsBody = SCNPhysicsBody.static()
        ...
        backSphereW.physicsBody = SCNPhysicsBody.static()
        ...
        let board = scene2.rootNode.childNode(withName: "Board", recursively: true)!
        ...
        let tree = scene3.rootNode.childNode(withName: "tree", recursively: true)
...
extension MyTestScene {
    ...
    func duplicateNode(_ node: SCNNode, material:SCNMaterial) -> SCNNode
    {
...
extension MyTestScene {
    ...
    func moveCamera(_ matrix: SCNMatrix4, x: Float, y: Float, z: Float)->SCNMatrix4
    {
...
extension MyTestScene {
    ...
    func setUpControllerObservers(){
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(MyTestScene.connectControllers), name: NSNotification.Name.GCControllerDidConnect, object: nil)
        
        NotificationCenter.default.addObserver(self, selector: #selector(MyTestScene.controllersDisconnected), name: NSNotification.Name.GCControllerDidDisconnect, object: nil)
    }
