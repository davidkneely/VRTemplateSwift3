...
class VRBaseScene : NSObject, VRControllerProtocol, SCNPhysicsContactDelegate {
    ...
    required override init() {
        ...
        timer = Timer.scheduledTimer(timeInterval: 0.1, target: self, selector: #selector(MyTestScene.countUp), userInfo: nil, repeats: true)
    ...
    func playerDidFocus(){
        ...
        if let pos = focusedNode?.position {
            ...
            let action = SCNAction.move(to: pos, duration: 0.2)
    ...
    func bulletDidHitDestroyable(_ node: SCNNode, with: String){
        ...
        let action = SCNAction.move(by: SCNVector3Make(0, 3, 0), duration: 0.2)
    ...
    func prepareFrame(with headTransform: GVRHeadTransform) {
        ...
        let actionH = SCNAction.rotate(by: CGFloat(GLKMathDegreesToRadians(rotateBy)), around: SCNVector3Make(0, 1, 0), duration: 0)
        ...
        if usingExtendedGamePad == true {
            ...
            if moveAccordingToHeadRotation == true {
                ...
                let p3 = cameraNode.convertPosition(p2, to: scene.rootNode)
                ...
                let action = SCNAction.moveBy(x: CGFloat((projected.x * moveZ) * multiplier), y: 0, z: CGFloat((projected.z * moveZ) * multiplier), duration: 0)
                world.runAction(action)
         
                if moveX > 0.1 || moveX < -0.1 {
                var vectorX = SCNVector3Make(-moveX, 0, 0)
                vectorX = cameraNode.convertPosition(vectorX, to: scene.rootNode)
                vectorX = multipliedByRotation(vectorX, rotation: rot)
                let actionX = SCNAction.moveBy(x: CGFloat(vectorX.x), y: 0, z: 0, duration: 0)
                world.runAction(actionX)
                }
                //
                if moveY > 0.1 || moveY < -0.1 {
                    var vectorY = SCNVector3Make(0, -moveY, 0)
                    let actionY = SCNAction.moveBy(x: 0, y: CGFloat(vectorY.y), z: 0, duration: 0)
        ...
        let newPosition = self.scene.rootNode.convertPosition(cameraNode.position, to: world)
        ...
        if control?.rightTriggerPressed == true {
            let pos = positionLine
            shootBullet(pos, to: projected, color: UIColor.red, size: 0.25, name: "bullet")
        }
        
        if control?.leftTriggerPressed == true {
            let pos = positionLine
            shootBullet(pos, to: projected, color: UIColor.blue, size: 0.25, name: "grappling hook")
        }
        
        /////////////////////////////////////
        let hits = things.hitTestWithSegment(from: position, to: p2, options: [SCNHitTestOption.firstFoundOnly.rawValue: true]);
        //MARK:FOCUS TEST
        ...
        things.enumerateChildNodes { (node, end) in
            node.geometry?.materials.first?.emission.contents = UIColor.clear
        };

        if let pos = focusedNode?.position {

            sphereFocusNode.geometry = focusedNode?.geometry
            sphereFocusNode.geometry?.materials.first?.emission.contents = UIColor.yellow.withAlphaComponent(0.3)
    ...
    func shootBullet(_ from: SCNVector3, to:SCNVector3, color: UIColor, size : CGFloat, name: String) {
        ...
        let delay = SCNAction.wait(duration: 1)
        let shootAction = SCNAction.move(to: to, duration: 2)
        let seqBullet = SCNAction.sequence([shootAction, delay, remove])
        bulletNode.runAction(seqBullet)
        
    }
    
    func buttonBPressed(_ from: SCNVector3, projected: SCNVector3){
    ...
    func doAdditionalUpdate(_ headTransform: GVRHeadTransform){
    ...
    func setUpScene(){
        ...
        floorBox.physicsBody = SCNPhysicsBody.static()
        ...
        sphere.materials.first?.specular.contents = UIColor.white
...
extension VRBaseScene {
    ...
    //MARK: PHYSICS
    /////////////////////////////////////////////////////////////
    func isCollisionBetween(_ nodeTypeOne: CC, nodeTypeTwo: CC, contact: SCNPhysicsContact)->Bool {
...
extension VRBaseScene {
    ...
    func handleKeyboardEvents(_ input: String) {
...
extension VRBaseScene {
    ...
    func multipliedByRotation(_ position: SCNVector3, rotation: SCNVector4)-> SCNVector3 {
    ...
    func moveCamera(_ node: SCNNode, x: Float, y: Float, z: Float)->SCNMatrix4
    {
...
extension VRBaseScene {
    ...
    func setUpControllerObservers(){
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(VRBaseScene.connectControllers), name: NSNotification.Name.GCControllerDidConnect, object: nil)
        
        NotificationCenter.default.addObserver(self, selector: #selector(VRBaseScene.controllersDisconnected), name: NSNotification.Name.GCControllerDidDisconnect, object: nil)
    }
