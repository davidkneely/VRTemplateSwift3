...
class VRScene_01: VRBaseScene {
    ...
    override func handleKeyboardEvents(_ input: String) {
    ...
    //MARK:UPDATE
    override func doAdditionalUpdate(_ headTransform: GVRHeadTransform) {
    ...
    func makeBox(){
        ...
        boxNode.position = cameraNode.convertPosition(spawnPosNode.position, to: things)
    ...
    func projectBox(_ distance: Float, node: SCNNode){
        print(distance)
        ...
        let action = SCNAction.move(by: scaledNormal, duration: 1)
        let runBlock = SCNAction.run { (node) in
            self.isMaking = false
    ...
    override func buttonBPressed(_ from: SCNVector3, projected: SCNVector3){
    ...
    override func doAdditionalSetup() {
        ...
        if sceneHasFog == true {
            self.scene.fogColor = UIColor.gray.withAlphaComponent(0.3)
            self.scene.fogStartDistance = 100
            self.scene.fogEndDistance = 250
            self.scene.fogDensityExponent = 0.8
        }
        
        setUpCompassPoints(things, backgroundContents: backgroundContents as AnyObject, distance: 150.0, sizeRadius: 5.0)
        setUpCompassPoints(things, backgroundContents: backgroundContents as AnyObject, distance: 50.0, sizeRadius: 5.0)
        ...
        let wave = sceneWave!.rootNode.childNode(withName: "wave", recursively: true)
    ...
    func setUpModels(){
        ...
        let barrel = scene.rootNode.childNode(withName: "barrel", recursively: true)
        ...
        barrelNode.physicsBody = SCNPhysicsBody.kinematic()
        //world.addChildNode(barrelNode)
        
        let structure = scene.rootNode.childNode(withName: "structure", recursively: true)
        ...
        let rock = scene.rootNode.childNode(withName: "rock", recursively: true)
        ...
        rockNode.physicsBody = SCNPhysicsBody.kinematic()
        //world.addChildNode(rockNode)
        
        let tree = treeScene.rootNode.childNode(withName: "tree", recursively: true)
        ...
        treeNode.physicsBody = SCNPhysicsBody.kinematic()
        //world.addChildNode(treeNode)
        
        let bamboo_1 = scene.rootNode.childNode(withName: "bambooCluster", recursively: true)
        ...
        let fern = scene.rootNode.childNode(withName: "fern", recursively: true)
        ...
        let lily = scene.rootNode.childNode(withName: "lily", recursively: true)
    ...
    func addShaders(){
        ...
        do {
            ...
            shaders[SCNShaderModifierEntryPoint.fragment] = try String(contentsOfFile: Bundle.main.path(forResource: "colorShader", ofType: "fsh")!, encoding: String.Encoding.utf8)
        ...
        if let shaders = shaders as? NSDictionary {
            ...
            if let shaderDict = shaders as? [SCNShaderModifierEntryPoint: String] {
