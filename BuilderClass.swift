...
class BuilderTools: SCNNode {
    ...
    //MARK: updateBUILDER
    func updateForBuilderMode(_ headTransform: GVRHeadTransform, control: ControlScheme, scene: VRBaseScene){
    ...
    func switchSelectedNode(_ increaseBy: Int){
    ...
    /////////////////////////////////////////////////////////
    func addToDictionary(_ smallNode: SmallNodeStruct){
        ...
        defaults.set(dictionaryOfSmallNodes, forKey: builderDictionaryKey)
    ...
    func printDictionaryOfSmallNodes(){
        ...
        let defaults = UserDefaults()
        print(defaults.dictionary(forKey: builderDictionaryKey))
    }
    ...
    /////////////////////////////////////////////////////////
    func previewPlaceNodeInLevel(_ pos: SCNVector3, selectedNodeTransform: SCNMatrix4){
        
        guard arrayOfNodesToBuildWith != nil else {return}
        
        var goToPosition = cameraNode!.convertPosition(pos, to: self)
    ...
    func placeNodeInLevel(_ pos: SCNVector3, selectedNodeTransform: SCNMatrix4){
        ...
        let nodeCopy: SCNNode = node.copy() as! SCNNode
        nodeCopy.position = cameraNode!.convertPosition(selNodePos, to: self)
        ...
        let action1 = SCNAction.move(to: goToPosition, duration: 0.3)
        let action2 = SCNAction.scale(to: 1, duration: 0.3)
        let action3 = SCNAction.rotateTo(x: 0, y: 0, z: 0, duration: 0.3)
        ...
        if let suffix = nodeCopy.name?.components(separatedBy: "_").last {
    ...
    func setPosAccordingToGrid(_ pos: SCNVector3)-> SCNVector3 {
        ...
        if abs(pos.x.truncatingRemainder(dividingBy: Float(increment))) > Float(Float(increment)/2) {
        ...
        if abs(pos.y.truncatingRemainder(dividingBy: Float(increment))) > Float(Float(increment)/2) {
        ...
        if abs(pos.z.truncatingRemainder(dividingBy: Float(increment))) > Float(Float(increment)/2) {
